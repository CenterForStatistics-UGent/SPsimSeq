...
rbind.fill2 <- function(...) {
  ...
  quickdf2(lapply(getters, function(x) x()))
}

#Compact list
compact2 <- function (l) Filter(Negate(is.null), l)

# Construct named lists of setters and getters.
output_template2 <- function(dfs, nrows) {
  ...
  for (df in dfs) {
    ...
    for (var in matching) {
      output[[var]] <- allocate_column2(df[[var]], nrows, dfs, var)
    }
...
make_assignment_call2 <- function(ndims) {
...
allocate_column2 <- function(example, nrows, dfs, var) {
  ...
  #construct an assignment expression like `column[rows, ...] <- what`
  #appropriate for the number of dims
  assignment <- make_assignment_call2(length(a$dim))
...
make_names2 <- function(x, prefix = "X") {
  nm <- names(x)
  if (is.null(nm)) {
    nm <- rep.int("", length(x))
  }
...
# system.time(rbind.fill2(mtcars[c("mpg", "wt")], mtcars[c("wt", "cyl")]))
# system.time(plyr::rbind.fill(mtcars[c("mpg", "wt")], mtcars[c("wt", "cyl")]))
