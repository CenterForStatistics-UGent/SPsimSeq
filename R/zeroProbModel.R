...
zeroProbModel <- function(count.data, n.mean.class=30, subset.data=FALSE, n.samples=400, ...){
  ...
  cpm.data <- log(edgeR::cpm(count.data)+1)
  ...
  #                       newdata=data.frame(x1=min(mean.log.cpm), x2=log.L)), ylim=c(0,1))
  # plot(mean.log.cpm, predict(fracZero.logit, type="response",
  #                            newdata=data.frame(x1=mean.log.cpm, x2=max(log.L))), ylim=c(0,1))
