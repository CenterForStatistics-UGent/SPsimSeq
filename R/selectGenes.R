...
selectGenesSim <- function(pDE, group, n.genes, null.genes0, nonnull.genes0,
  ...
  if(pDE>0 & !is.null(group) & length(group.config)>1){
    if((1-pDE)*n.genes <= length(null.genes0)){
      ...
      if(h==1){
        message("Note: The number of null genes (not DE) in the source data is ", length(null.genes0)," and the number of null genes required to be included in the simulated data is ", round((1-pDE)*(n.genes)), ". Therefore, candidiate null genes are sampled with replacement. \n This message applies to the rest of the simulatons.")
      }
    ...
    if(length(nonnull.genes0)>0){
      if(pDE*n.genes <= length(nonnull.genes0)){
        ...
        if(h==1){
          message("Note: The number of DE genes detected in the source data is ", length(nonnull.genes0), " and the number of DE genes required to be included in the simulated data is ", round(pDE*n.genes), ". Therefore, candidiate DE genes are sampled with replacement. \n This message applies to the rest of the simulatons.")
        }
        #set.seed(setSimContol$seed.sample.genes[h]+1)
