...
checkInputValidity <- function(s.data, group, batch, group.config, batch.config){
  ...
  source.class <- is(s.data, "SingleCellExperiment") | is(s.data, "matrix") | is(s.data, "data.frame")
  ...
  if(!is.null(group)){
    if(length(group) != ncol(s.data)){
      val5  <- -1
      mes5 <-  "The length of the group variable is not conformable with the number of columns in the source count matrix. \n"
  ...
  if(!is.null(batch)){
    if(length(batch) != ncol(s.data)){
      val6  <- -1
      mes6 <-  "The length of the batch variable is not conformable with the number of columns in the source count matrix. \n"
  ...
  if(!is.null(group)){
    if(length(group.config) > length(unique(group))){
      val12  <- -1
      mes12 <-  "The number of groups to be simulated is larger than the number of groups available in the source data. \n"
    }
    else if((length(group.config) < length(unique(group))) & length(group.config) !=1){
      val12  <- -1
      mes12 <-  "The number of groups to be simulated is less than the number of groups available in the source data. Maybe subset your source data OR consider adding a 0 in the group.config vector for the group that you do not want to simulate from. Example group.config=c(0.5, 0.5, 0). \n"
    } 
    else if((length(unique(group))!=1) & length(group.config) ==1){
      val12  <- 1
      mes12 <-  paste("The source data has", length(unique(group)), "groups but you are simulating only for one group (length(group.config)=1). Hence, data only from one of the groups will be used. \n")
    } 
    else{val12 <- 0; mes12 <- "none"} 
  }else{
    if(length(group.config) > 1){
      val12  <- -1
      mes12 <- paste("You are attempting to simulate",  length(group.config), "groups but there is no groupping variable (group=NULL) for the source data. \n")
    } 
    else{val12 <- 0; mes12 <- "none"}
  } 
  ...
  if(!is.null(batch)){
    if(length(batch.config) != length(unique(batch))){
      val13  <- 1
      mes13 <- "The number of batchs to be simulated is not equal to the number of batches available in the source data. \n"
    }else{val13 <- 0; mes13 <- "none"}  
  }else{
    if(length(batch.config) > 1){
      val13  <- -1
      mes13 <- paste("You are attempting to simulate",  length(batch.config), "batches but there is no batch indicator (batch=NULL) for the source data. \n")
    }else{val13 <- 0; mes13 <- "none"}   
  }   
...
# v <- log2(u + 1)
# 
# coldat <- data.frame(id=1:ncells,  x=c(runif(ncells)), y=c(rep(1:ncells)))
# rowdat <- data.frame(id=1:nrow(u), xx=c(runif(nrow(u))))
# 
...
#  
# group=rep(c(0,1,2), 2*c(20, 20, 10))
# batch=NULL
# 
# procesDta <- function(s.data, group, batch, group.config, batch.config){
...
#     message(paste0("Error ", 1:length(msgs), "-- ", msgs)) 
#     if(length(msgs) > 1){
...
#     msgs <- checkInputs$mes[checkInputs$val==1]
#     message(paste0("Note ", 1:length(msgs), "-- ", msgs)) 
#     if(length(msgs) > 1){
...
# }
# 
# procesDta(s.data = sce, group = group, batch = NULL, 
#           group.config = c(0.5, 0, 0.5), 
#           batch.config = c(1))
