...
params <- setParams(params, update = list(batch.facLoc=0.5, batch.facScale=0.3, seed=263))
sim.batches <- splatSimulate(params, 
                             batchCells = c(50, 50, 50, 50, 50, 50, 50), 
                             verbose = FALSE)
...
sim.batches2 <- sim.batches[keep,]
 ...
 # - the cells are from a single experimental group (group.config = 1)
 # - cells are from 7 batchs each with equal number of cells (batch.config = rep(1/7, 7))
 # - we add 0% DE genes (pDE = 0) 
 # - we model the zeroes separately (model.zero.prob = TRUE) 
 # - the ouput will be in SingleCellExperiment class object (result.format = "SCE") 
 ...
 SPsimuBatch <- simulateData(s.data = sim.batches2, batch = batch.factor,
                             n.genes = 2000, batch.config = rep(1/7, 7), group.config=1,
                             tot.samples=350, model.zero.prob = TRUE)
 SPsimuBatch[[1]] 
 ...
 colData(SPsimuBatch[[1]])$Batch <- as.factor(colData(SPsimuBatch[[1]])$Batch) 
